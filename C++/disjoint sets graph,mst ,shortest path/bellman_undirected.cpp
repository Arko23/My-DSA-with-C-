...
void bellman(vector<ed> &edge, int n, int s)
{
    ...
    for (int i = 0; i < n - 1; i++)
    {
        for (auto &x : edge)
        {
            // Consider both directions of the edge
            if (dist[x.src] != INT_MAX && dist[x.src] + x.w < dist[x.des])
            {
                dist[x.des] = dist[x.src] + x.w;
            }
            if (dist[x.des] != INT_MAX && dist[x.des] + x.w < dist[x.src])
            {
                dist[x.src] = dist[x.des] + x.w;
            }
        }
    }
...
// Enter edge 3 (source destination weight): 2 1 3  // Reversed direction of edge 3
// Enter edge 4 (source destination weight): 1 3 2
// Enter edge 5 (source destination weight): 1 3 1  // Same edge as edge 4, but different direction

...
void sortdup()
{
    ...
    while (cur->next != NULL)
    {
        ...
        while (ncur != NULL)
        {
            if (cur->value > ncur->value)
            {
                temp = cur->value;
                cur->value = ncur->value;
                ncur->value = temp;
            }
            ncur = ncur->next;
        }
...
int main()
{
    ...
    while (1)
    {
        printf("\n1. Insert new item. 2. Delete item. 3. Search item. 4. Insert Last. 5. Print. 6.Delete Last 7.Delete First 8.Insert After 9.Print Linked List in Reverse way. 10.Sum 11.Make the linked list reverse 12.Sort and find duplicates 14.Destruct all nodes 15.Exit\n--------------------------------------------------------------------------------------\n");
